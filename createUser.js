...
fs.readFile('/etc/hotspot/users.json', { encoding: 'utf8' }, function (err, data) {
	...
	fs.writeFile('/etc/hotspot/users.json', JSON.stringify(users), { encoding: 'utf8' }, function(){
		...
		console.log('Valid for', MACs, 'MACs');
		console.log('Valid until', expiration.getDate() + '/' + expiration.getMonth() + '/' + expiration.getFullYear(), expiration.getHours() + ':' + expiration.getMinutes() + ':' + expiration.getSeconds())
	})
});